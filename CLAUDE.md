...
- **⛔ NO HAY COMPILACIÓN EN TIEMPO DE EJECUCIÓN QUE FUNCIONE DENTRO DE WILDFLY.** Verificado el 2026-08-25 listando el contenido de los jar desplegados. En JasperReports **7.0.3** `JRJaninoCompiler` **no existe** —Janino se retiró del producto— y `JRJdtCompiler` vive en el artefacto aparte `jasperreports-jdt`, que **no está, ni debe estar, en el `pom.xml` del WAR**. Lo único que el jar principal trae es `JRJavacCompiler` y `JRJdk13Compiler`, los dos con compilador externo y sin acceso al classloader del deployment. **Cualquier valor de `net.sf.jasperreports.compiler.class` da el mismo `Could not instantiate report compiler` dentro de WildFly** cambiando sólo el nombre de la clase — se han probado los dos y se ha perdido una tarde en cada uno. Esto sigue siendo cierto y no cambia: el `.jasper` se produce **fuera** de WildFly, nunca en el servidor.
- **Por eso el `.jasper` no es opcional: es parte de añadir un reporte.** `ReporteServiceImpl:110` busca `/rep/{modulo}/{nombre}.jasper` y **sólo cae al `.jrxml` si no lo encuentra** — y ese respaldo está muerto (ver el punto anterior). **Un reporte con sólo `.jrxml` compila en el IDE, pasa la revisión, entra en el commit y revienta la primera vez que un usuario lo ejecuta.** Fue exactamente lo que pasó con los siete de `rhh`, que se añadieron sin compilar mientras `cnt`, `crd`, `cxc` y `tsr` sí llevaban su par.
- **Ya no hace falta Jaspersoft Studio para generar el `.jasper`** (corregido 2026-09-08 — hasta esa fecha esta sección decía que sí, y en la OMEN no hay Studio instalado). `jasperreports-jdt:7.0.3` (el mismo Eclipse Compiler for Java que usa Studio por dentro) se resuelve por Maven sin problema; lo que no hay es un lugar seguro para ponerlo — meterlo en el `pom.xml` del WAR reintroduciría el compilador que **si** funciona fuera del servidor pero está prohibido llevar al deployment. La solución es `tools/jasper/`: un módulo Maven aparte (no es parte del reactor del WAR, no se empaqueta con la app) con su propio `pom.xml` que sí declara `jasperreports-jdt`, más `tools/jasper/src/tools/jasper/CompilarJasper.java`, un `main` que compila con `JasperCompileManager.compileReportToFile`. **El criterio de "falta compilar" es deliberadamente por existencia, no por fecha de modificación**: comparar mtimes recompilaría de más justo después de un `git clone`/`checkout` (todos los archivos quedan con el mismo mtime, en orden arbitrario), metiendo en el commit `.jasper` ajenos que otro equipo ya había generado bien. Por eso, sin argumentos solo compila los `.jrxml` que **no tienen** `.jasper` todavía; para recompilar uno que se acaba de editar, pasale la ruta explícita (`compilar-jasper.bat src/main/resources/rep/tsr/RPRT_CNCL_CNTA.jrxml`), que sí se recompila siempre; `--forzar` recompila todo bajo `rep/` (para un cambio de versión de JasperReports, no para el uso diario). **Al añadir o modificar un `.jrxml`, correr `compilar-jasper.bat <ruta-al-jrxml>` desde la raíz del repo y commitear el `.jrxml` y el `.jasper` juntos.** Y si un reporte usara subreportes, los subreportes también van compilados: `SUBREPORT_DIR` apunta a la carpeta del módulo y el motor los carga por `.jasper`.
- **⛔ COMPILAR NO ALCANZA. Hace falta ADEMÁS pasar el fill de prueba.** Incidente real, 2026-09-08: `RPRT_CNCL_CNTA`/`RPRT_CNCL_GNRL` compilaron sin ningún aviso y reventaron en producción con `JRRuntimeException: Could not resolve style(s): .` la primera vez que un usuario generó el PDF. Causa: un `<style>` anónimo anidado dentro de un `<element>` en vez de un `<style name="...">` a nivel de reporte referenciado con `style="Nombre"` — **el compilador no valida referencias de estilo**, solo la sintaxis; esa clase de error (estilos, subreportes rotos, expresiones que revientan) SOLO aparece al llenar el reporte. `tools/jasper/src/tools/jasper/VerificarFill.java` + `verificar-fill-jasper.bat <ruta-al-jasper>` llenan el `.jasper` con `JasperFillManager.fillReport(reporte, paramsDummy, new JREmptyDataSource())` — sin Oracle, sin una sola fila, pero corriendo exactamente la misma resolución de estilos/subreportes/expresiones que un fill real. Los parámetros se rellenan con valores dummy según su clase declarada; si un reporte depende de un subreporte con conexión JDBC real, el verificador lo distingue como "NO VERIFICABLE (necesita conexión)" en vez de darlo por bueno a ciegas. **Al añadir o modificar un `.jrxml`: `compilar-jasper.bat <ruta>` Y `verificar-fill-jasper.bat <ruta-al-jasper>`, los dos, antes de comittear.** (Barrido 2026-09-08 de los 58 `.jasper` del repo con este verificador, ya con el fix del punto siguiente: 56 OK, 2 NO VERIFICABLE, 0 fallidos — los 2 no verificables son `rep/test/naturaleza_cuenta_empresa.jasper` y el `RPRT_ASNT_CNTB.jasper` de esa misma carpeta de prueba, ambos de commits viejos y ajenos a esta tarea, sin tocar; quien sea dueño de esos archivos debería revisarlos.)
- **⛔ `set /p VARIABLE=<archivo` trunca SILENCIOSAMENTE alrededor de los 1024 caracteres en Windows `cmd.exe`, sin ningún error.** Encontrado el 2026-09-08 de la forma más cara posible: `verificar-fill-jasper.bat` reportaba 5 RIDE de `cxc` y un reporte de `crd` como "rotos" (`Class not found` / falla evaluando `UPPER(...)`) — pero SOLO al correr el `.bat`; los mismos `.jasper`, con el mismo classpath, cargados a mano en el mismo JDK, pasaban limpio. Medido con `System.getProperty("java.class.path")`: el classpath real (39 jars, ~3700 caracteres) llegaba con **12 entradas** cuando se leía con `set /p CLASSPATH=<"%CP_FILE%"`, y con las **39 completas** leyéndolo con `for /f "usebackq delims=" %%C in ("%CP_FILE%") do set "CLASSPATH=%%C"`. Como `mvn dependency:build-classpath` pone `jasperreports-jdt`, `ecj`, `jasperreports-functions`, `jasperreports-barcode4j` y `zxing` **al final** de la lista, eran justo esos los que se caían — ni un defecto de los reportes ni de JasperReports, un límite de `cmd.exe` que nadie documenta. **Los dos `.bat` de `tools/jasper` ya usan `for /f`, no `set /p`, para leer el classpath — si alguna vez se toca ese paso, no volver a `set /p` con un archivo que pueda superar ~1000 caracteres.**
- **⛔ EXISTEN DOS SINTAXIS DE `.jrxml` Y SOLO UNA SE PUEDE VOLVER A COMPILAR CON LO QUE TRAE ESTE REPO.** JasperReports 7.0.3 admite dos dialectos de XML: la "clásica" (`xmlns="http://jasperreports.sourceforge.net/jasperreports"`, `<textField><reportElement .../><textElement>...<textFieldExpression class="...">...`) y la "compacta" (`<jasperReport name="..." language="java">` sin `xmlns`, `<element kind="textField|staticText|line|image" x= y= .../><expression>...</expression></element>`). Un `.jasper` YA COMPILADO de cualquiera de las dos carga y llena perfecto — es deserialización binaria, no le importa el dialecto de origen — pero **recompilar un `.jrxml` clásico revienta con `JRException: Unable to load report` sin causa ni log, con cualquier classpath**. No es un jar que falte ni una versión de dependencia: se confirmó bajando el jar-with-sources (`mvn dependency:get -Dartifact=net.sf.jasperreports:jasperreports:7.0.3:jar:sources`) y leyendo `JRXmlLoader.loadXML` — itera `jasperReportsContext.getExtensions(ReportLoader.class)`, y el **único** `ReportLoader` que trae el jar open-source de 7.0.3 es `JacksonReportLoader`, que solo entiende la sintaxis compacta. No hay loader clásico en absoluto en esta versión, con ningún classpath ni ninguna versión de woodstox. Pasó el 2026-09-08 con los seis balances de `rep/cnt/`: cinco de los seis `.jrxml` estaban en sintaxis clásica (quedaron así de una generación con Jaspersoft Studio vieja) y no compilaban con nada hasta convertirlos. **Al modificar un `.jrxml` clásico, primero convertirlo a sintaxis compacta** — mecánico: mismos elementos, mismas coordenadas, misma `query` y columnas, solo cambia la serialización (`reportElement`/`textElement`/`textFieldExpression` → atributos planos en `<element kind="...">`); ver `RPRTCNTB_ACUM_CNFI.jrxml` como ejemplo ya convertido, con el comentario de cabecera explicando el porqué. Un `.jrxml` que ya está en sintaxis compacta (se distingue por la ausencia de `xmlns` y por `<element kind="...">`) no necesita conversión.
