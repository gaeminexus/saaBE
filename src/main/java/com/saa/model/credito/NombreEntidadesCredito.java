package com.saa.model.credito;

public interface NombreEntidadesCredito {
	String BOT_OPCION = "BotOpcion";
	String CANTON = "Canton";
	String CREDITO_MONTO_APROBACION = "CreditoMontoAprobacion";
	String ESTADO_CESANTIA = "EstadoCesantia";
	String ESTADO_CIVIL = "EstadoCivil";
	String ESTADO_PARTICIPE = "EstadoParticipe";
	String ESTADO_PRESTAMO = "EstadoPrestamo";
	String EXTER = "Exter";
	String TIPO_PRESTAMO = "TipoPrestamo";
	String FILIAL = "Filial";
	String TIPO_VIVIENDA = "TipoVivienda";
	String TIPO_HIDROCARBURIFICA = "TipoHidrocarburifica";
	String TIPO_IDENTIFICACION = "TipoIdentificacion";
	String ENTIDAD = "Entidad";
	String PRODUCTO = "Producto"; 
	String PRESTAMO = "Prestamo";
	String DETALLE_PRESTAMO = "DetallePrestamo";
	String PAGO_PRESTAMO = "PagoPrestamo";
	String PARTICIPE = "Participe";
	String APORTE = "Aporte";
	String CONTRATO = "Contrato";
	String METODO_PAGO = "MetodoPago";
	String NIVEL_ESTUDIO = "NivelEstudio";
	String PROFESION = "Profesion";
	String PAIS = "Pais";
	String TIPO_APORTE = "TipoAporte";
	String TIPO_CALIFICACION_CREDITO = "TipoCalificacionCredito";
	String TIPO_CONTRATO = "TipoContrato";
	String TIPO_CESANTIA = "TipoCesantia";
	String TIPO_ADJUNTO = "TipoAdjunto";
	String TIPO_GENERO = "TipoGenero";
	String TIPO_PAGO = "TipoPago";
	String TIPO_PARTICIPE = "TipoParticipe";
	String TIPO_REQUISITO_PRESTAMO = "TipoRequisitoPrestamo";
	String MOTIVO_PRESTAMO = "MotivoPrestamo";
	String PROVINCIA = "Provincia";
	String MORA_PRESTAMO = "MoraPrestamo";
	String PERSONA_NATURAL = "PersonaNatural";
	String BIO_PROFILE = "BioProfile";
	String TASA_PRESTAMO = "TasaPrestamo";
	String CIUDAD = "Ciudad";
	String DOCUMENTO_CREDITO = "DocumentoCredito";
	String PARROQUIA = "Parroquia";
	String HISTORIAL_SUELDO = "HistorialSueldo";
	String CAMBIO_APORTE = "CambioAporte";
	String CESANTIA = "Cesantia";
	String DIRECCION = "Direccion";
	String CXC_PARTICIPE = "CxcParticipe";
	String DATOS_PRESTAMO = "DatosPrestamo";
	String RELACION_PRESTAMO = "RelacionPrestamo";
	String REQUISITOS_PRESTAMO = "RequisitosPrestamo";
	String ADJUNTO = "Adjunto";
	String COMENTARIO = "Comentario";
	String PERFIL_ECONOMICO = "PerfilEconomico";
	String DIRECCION_TRABAJO = "DireccionTrabajo";
	String PAGO_APORTE = "PagoAporte";
	String CXC_KARDEX_PARTICIPE = "CxcKardexParticipe";
	String CARGA_ARCHIVO = "CargaArchivo";
	String DETALLE_CARGA_ARCHIVO = "DetalleCargaArchivo";
	String PARTICIPE_X_CARGA_ARCHIVO = "ParticipeXCargaArchivo";
	String AUDITORIA = "Auditoria";
	String PARTICIPE_ASOPREP = "ParticipeAsoprep";
	String APORTE_ASOPREP = "AporteAsoprep";
	String TRANSACCIONES_ASOPREP = "TransaccionesAsoprep";
	

}

