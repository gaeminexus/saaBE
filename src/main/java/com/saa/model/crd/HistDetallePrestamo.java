...
/**
 * Representa la tabla HDTP (HistDetallePrestamo).
 *
 * Cuotas de CRD.DTPR historizadas al re-amortizar (abono a capital / precancelación /
 * reverso). Las cuotas reemplazadas se copian aquí y se BORRAN de DTPR en la misma
 * transacción: dejarlas vivas con un estado nuevo rompería
 * contarCuotasPendientesByPrestamo y todos los reportes que suman DTPR.
 *
 * Los campos del espejo llevan EXACTAMENTE los mismos nombres Java y de columna que
 * com.saa.model.crd.DetallePrestamo, para permitir la copia campo a campo y las consultas
 * comparativas. Verificado 1:1 contra DetallePrestamo el 2026-08-13.
 */
@SuppressWarnings("serial")
@Entity
@Table(name = "HDTP", schema = "CRD")
@NamedQueries({
    @NamedQuery(name = "HistDetallePrestamoAll", query = "select e from HistDetallePrestamo e"),
    @NamedQuery(name = "HistDetallePrestamoId", query = "select e from HistDetallePrestamo e where e.codigo = :id")
})
public class HistDetallePrestamo implements Serializable {

    /** Código del registro histórico */
    @Id
    @Basic
    @Column(name = "HDTPCDGO", precision = 0)
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Long codigo;

    /** Código que tenía la cuota en CRD.DTPR (sin FK: la fila viva se borra al historizar) */
    @Basic
    @Column(name = "DTPRCDGO")
    private Long codigoOriginal;

    /** FK - Evento que historizó la cuota */
    @ManyToOne
    @JoinColumn(name = "EVPRCDGO", referencedColumnName = "EVPRCDGO")
    private EventoPrestamo eventoPrestamo;

    /** Motivo: ABONO_CAPITAL | PRECANCELACION | REVERSO */
    @Basic
    @Column(name = "HDTPMTVO", length = 30)
    private String motivo;

    /** Fecha/hora en que se historizó la cuota */
    @Basic
    @Column(name = "HDTPFCRG")
    private LocalDateTime fechaRegistroHist;

    /** Usuario que ejecutó la operación que historizó la cuota */
    @Basic
    @Column(name = "HDTPUSAR", length = 50)
    private String usuarioHist;

    // ============================================================
    // Espejo 1:1 de CRD.DTPR (mismos nombres de campo y de columna)
    // ============================================================
    ...
    public LocalDateTime getFechaRegistroHist() {
        return fechaRegistroHist;
    }

    public void setFechaRegistroHist(LocalDateTime fechaRegistroHist) {
        this.fechaRegistroHist = fechaRegistroHist;
    }

    public String getUsuarioHist() {
        return usuarioHist;
    }

    public void setUsuarioHist(String usuarioHist) {
