...
/**
 * @author GaemiSoft
 * Pojo mapeo de tabla CBR.CCIC.
 * Entity ComposicionCuotaInicialCobro.
 * Composicion de la cuota inicial para cobros. 
 * Determina qué valores del documento se incluiran en la cuota inicia. 
 * Los valores se los toma de la entidad de resumen de valores por documento (RVDC).
 */
@SuppressWarnings("serial")
@Entity
@Table(name = "CCIC", schema = "CBR")
@NamedQueries({
    @NamedQuery(name = "ComposicionCuotaInicialCobroAll", query = "select e from ComposicionCuotaInicialCobro e"),
    @NamedQuery(name = "ComposicionCuotaInicialCobroId", query = "select e from ComposicionCuotaInicialCobro e where e.codigo = :id")
})
public class ComposicionCuotaInicialCobro implements Serializable {
    ...
    @Id
    @Basic
    @Column(name = "CCICCDGO", precision = 0)
     ...
     */
    @ManyToOne
    @JoinColumn(name = "RVDCCDGO", referencedColumnName = "RVDCCDGO")
    private ResumenValorDocumentoCobro resumenValorDocumentoCobro;
    ...
    public ResumenValorDocumentoCobro getResumenValorDocumentoCobro() {
        return resumenValorDocumentoCobro;
    }

    public void setResumenValorDocumentoCobro(ResumenValorDocumentoCobro resumenValorDocumentoCobro) {
        this.resumenValorDocumentoCobro = resumenValorDocumentoCobro;
    }
