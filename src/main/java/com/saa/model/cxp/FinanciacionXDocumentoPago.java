...
/**
*  @author GaemiSoft
*  Pojo mapeo de tabla PGS.FXDP.
*  Entity FinanciacionXDocumentoPago.
*  Financiacion por documento de pago. Contiene la financiacion de un documento.
*/
@SuppressWarnings("serial")
@Entity
@Table(name = "FXDP", schema = "PGS")
@SequenceGenerator(name = "SQ_FXDPCDGO", sequenceName = "PGS.SQ_FXDPCDGO", allocationSize = 1)
@NamedQueries({
	@NamedQuery(name = "FinanciacionXDocumentoPagoAll", query = "select e from FinanciacionXDocumentoPago e"),
	@NamedQuery(name = "FinanciacionXDocumentoPagoId", query = "select e from FinanciacionXDocumentoPago e where e.codigo = :id")
})
public class FinanciacionXDocumentoPago implements Serializable {

	/**
	 * Codigo de la entidad.
	 */
	@Id
	@Column(name = "FXDPCDGO", precision = 0)
	@GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "SQ_FXDPCDGO")
	private Long codigo;
	 ...
	 */
	@Basic
	@Column(name = "FXDPDPOF")
	private Long dependeOtraFinanciacion;
	
	/**
	 * Numero de documento del que depende. 
	 * Tomado del campo numeroDocumentoString de la entidad DocumentoPago.
	 */
	@Basic
	@Column(name = "FXDPNDCD")
