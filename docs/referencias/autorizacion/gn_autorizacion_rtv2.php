...
include "../reports/fn_rprt_rtv2.php";
include "gn_mail_rtv2.php";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){    
    ...
    $xml->asXml('../../resources/'.$idFacturador.'/rtv2/f/'.$clave.'.xml');
    // Inserta el path para angular en la tabla de pathRetencion
    $pathAngular='resources/'.$idFacturador.'/rtv2/f/'.$clave.'.xml';
    $sql = "INSERT INTO prt2 (id, retencionv2, path, alterno)
    ...
    if($conectaSRI == 1){
        ...
        try {
            $file_ws1 = fopen('../../resources/'.$idFacturador.'/rtv2/e/'.$clave.'.txt', "w");
            $contenido = file_get_contents('../../resources/'.$idFacturador.'/rtv2/f/'.$clave.'.xml');
            ...
            file_put_contents('../../resources/'.$idFacturador.'/rtv2/e/'.$clave.'.xml', $contenido);
            // Inserta el path para angular en la tabla de pathRetencion
            $pathAngular='resources/'.$idFacturador.'/rtv2/e/'.$clave.'.xml';
            $sql = "INSERT INTO prt2 (id, retencionv2, path, alterno)
            ...
            if($result->RespuestaRecepcionComprobante->estado == 'RECIBIDA'){
                ...
                try {
                    ...
                    if($estadoXml == 'AUTORIZADO'){                        
                        $file_ws2_a = fopen('../../resources/'.$idFacturador.'/rtv2/a/'.$clave.'.txt', "w");
                        ...
                        $xml->asXml('../../resources/'.$idFacturador.'/rtv2/a/'.$clave.'.xml');                        
                        // Inserta el path para angular en la tabla de pathRetencion
                        $pathAngular='resources/'.$idFacturador.'/rtv2/a/'.$clave.'.xml';
                        $sql = "INSERT INTO prt2 (id, retencionv2, path, alterno)
                        ...
                        if($ambiente==2){
                            if($_POST['destinatario']!=null){                                
                                ...
                                //ENVIAR MAIL
                                $v_resultado = enviar_mail_rtv2($_POST['destinatario'], $idFacturador, $clave, $logo);
                                $respuesta = $respuesta." ".$v_resultado;
                            }
                        }                        
                    }else{
                        // NO AUTORIZADA estadoRetencion=6;
                        // file_put_contents('../../resources/'.$idFacturador.'/rtv2/n/'.$clave.'.xml', $contenido);
                        ...
                        $xml->asXml('../../resources/'.$idFacturador.'/rtv2/n/'.$clave.'.xml');
                        $file_ws2_n = fopen('../../resources/'.$idFacturador.'/rtv2/n/'.$clave.'.txt', "w");
                        fwrite($file_ws2_n, "Respuesta WS2: " . print_r($result,true) . PHP_EOL);
                        // Inserta el path para angular en la tabla de pathRetencion
                        $pathAngular='resources/'.$idFacturador.'/rtv2/n/'.$clave.'.xml';
                        $sql = "INSERT INTO prt2 (id, retencionv2, path, alterno)
                ...
                if(strstr($mensaje, "CLAVE ACCESO REGISTRADA") !== false){
                    ...
                    $sql = "UPDATE rtv2 SET autorizacion = '$clave', fechaAutorizacion = addtime(fecha, '00:01:15'),
