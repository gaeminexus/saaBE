...
include "../reports/fn_rprt_ntcr.php";
include "gn_mail_ntcr.php";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){    
    ...
    $clave = $_POST['clave'];
    $codigoNotaCredito = $_POST['idNotaCredito'];
    $respuesta ='';    
    //Grabar XML
    $strXML='<?xml version="1.0" encoding="UTF-8"?>'.$_POST['xml'];
    $xml = new SimpleXMLElement($strXML);
    $xml->asXml('../../resources/'.$idFacturador.'/ntcr/f/'.$clave.'.xml');
    // Inserta el path para angular en la tabla de pathNotaCredito
    $pathAngular='resources/'.$idFacturador.'/ntcr/f/'.$clave.'.xml';
    $sql = "INSERT INTO ptnc (id, notaCredito, path, alterno)
            VALUES (0, $codigoNotaCredito, '$pathAngular', 3)";
    $statement = $dbConn->prepare($sql);
    $statement->execute();
    // FIRMADA estadoNotaCredito=3;
    $sql = "UPDATE ntcr
        SET estado = 3
        WHERE clave='$clave'";
    $statement = $dbConn->prepare($sql);
    ...
    if($conectaSRI == 1){
        ...
        try {
            $file_ws1 = fopen('../../resources/'.$idFacturador.'/ntcr/e/'.$clave.'.txt', "w");
            $contenido = file_get_contents('../../resources/'.$idFacturador.'/ntcr/f/'.$clave.'.xml');
            ...
            file_put_contents('../../resources/'.$idFacturador.'/ntcr/e/'.$clave.'.xml', $contenido);
            // Inserta el path para angular en la tabla de pathNotaCredito
            $pathAngular='resources/'.$idFacturador.'/ntcr/e/'.$clave.'.xml';
            $sql = "INSERT INTO ptnc (id, notaCredito, path, alterno)
                    VALUES (0, $codigoNotaCredito, '$pathAngular', 4)";
            $statement = $dbConn->prepare($sql);
            $statement->execute();
            // ENVIADA estadoNotaCredito=4;
            $sql = "UPDATE ntcr
            SET estado = 4
            WHERE clave='$clave'";
            $statement = $dbConn->prepare($sql);
            ...
            if($result->RespuestaRecepcionComprobante->estado == 'RECIBIDA'){
                ...
                try {
                    ...
                    if($estadoXml == 'AUTORIZADO'){                        
                        $file_ws2_a = fopen('../../resources/'.$idFacturador.'/ntcr/a/'.$clave.'.txt', "w");
                        ...
                        $xml->asXml('../../resources/'.$idFacturador.'/ntcr/a/'.$clave.'.xml');                        
                        // Inserta el path para angular en la tabla de pathNotaCredito
                        $pathAngular='resources/'.$idFacturador.'/ntcr/a/'.$clave.'.xml';
                        $sql = "INSERT INTO ptnc (id, notaCredito, path, alterno)
                                VALUES (0, $codigoNotaCredito, '$pathAngular', 5)";
                        $statement = $dbConn->prepare($sql);
                        $statement->execute();
                        // AUTORIZADA estadoNotaCredito=5;
                        ...
                        fclose($file_ws2_a);
                        $respuesta = $estadoXml;
                        crearPDF_ntcr($dbConn, $clave, 1);                        
                        if($ambiente==2){
                            if($_POST['destinatario']!=null){                                
                                $logo = "https://valeecuador.com/fel/".$_POST['pathLogo'];
                                //ENVIAR MAIL
                                $v_resultado = enviar_mail_ntcr($_POST['destinatario'], $idFacturador, $clave, $logo);
                                $respuesta = $respuesta." ".$v_resultado;
                            }
                        }                        
                    }else{
                        // NO AUTORIZADA estadoNotaCredito=6;
                        // file_put_contents('../../resources/'.$idFacturador.'/ntcr/n/'.$clave.'.xml', $contenido);
                        ...
                        $xml->asXml('../../resources/'.$idFacturador.'/ntcr/n/'.$clave.'.xml');
                        $file_ws2_n = fopen('../../resources/'.$idFacturador.'/ntcr/n/'.$clave.'.txt', "w");
                        fwrite($file_ws2_n, "Respuesta WS2: " . print_r($result,true) . PHP_EOL);
                        // Inserta el path para angular en la tabla de pathNotaCredito
                        $pathAngular='resources/'.$idFacturador.'/ntcr/n/'.$clave.'.xml';
                        $sql = "INSERT INTO ptnc (id, notaCredito, path, alterno)
                                VALUES (0, $codigoNotaCredito, '$pathAngular', 6)";
                        $statement = $dbConn->prepare($sql);
                        $statement->execute();
                        // NO AUTORIZADA estadoNotaCredito=6;
                        $sql = "UPDATE ntcr
                ...
                if(strstr($mensaje, "CLAVE ACCESO REGISTRADA") !== false){
                    ...
                    $sql = "UPDATE ntcr SET autorizacion = '$clave', fechaAutorizacion = addtime(fecha, '00:01:15'),
