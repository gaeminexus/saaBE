...
## Publicado el 2026-08-20, y verificado en vivo

El filtro salió con la tanda del día. No se pudo escalonar como decía el apartado anterior —no se
publica medio WAR y la clase ya estaba en el árbol—; queda asumido y el arreglo del lado del
frontend está pedido. La única pantalla afectada sigue siendo la de conciliación contable de TSR.

**La comprobación que faltaba, hecha: hay cuerpos que ya son JSON, y el filtro los respeta.**
Son **14 sitios en cuatro clases** —`FormaPagoFacturaRest`, `FormaPagoLiquidacionRest`,
`EjecucionReporteRest` y `EjecucionReporteCarteraRest`—, todos con la forma
`.entity("{\"error\":\"" + e.getMessage() + "\"}")`. Comprobado contra el desplegado: un error de
`formas-pago-factura` sale tal cual, sin envolver dos veces. La guarda del `{` funciona.

> **De paso, un defecto de esos 14 sitios que conviene registrar:** construyen el JSON
> concatenando cadenas, así que un `getMessage()` con una comilla o un salto de línea produce un
> cuerpo **inválido**. Es exactamente el fallo que este filtro evita serializando un `Map`. No se
> tocaron —no es este trabajo— pero el día que uno de esos mensajes lleve comillas, el cliente no
> podrá parsearlo.

**Y el filtro hace lo que debe.** Verificado con un error cuyo mensaje lleva **comillas dobles y
saltos de línea** —un criterio de búsqueda malformado, que devuelve el JPQL entero en el
mensaje—: el cuerpo llega como JSON válido, parseado con un parser real, con la clave `mensaje`,
las comillas escapadas y **el mensaje repartido en tres líneas intactas**. Es el caso que motivó
serializar un `Map` en vez de concatenar.

Comprobado también que **actúa en 4xx y no solo en 5xx** (un 404 de `myan/resultado` sale
envuelto), y que la traza `MensajeErrorJsonFilter envuelve un error NNN de /ruta` deja rastro en
el log de cada envoltura.
