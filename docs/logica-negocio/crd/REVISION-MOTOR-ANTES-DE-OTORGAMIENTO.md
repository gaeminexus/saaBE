...
**`sql/88_BACKFILL_PRSTINNM_DESDE_PRSTTSAA.sql`**.

#### ⚠️ Medido contra la base — y el resultado cambia la conclusión

El bloque de diagnóstico de `sql/88` se corrió contra la base local el **2026-08-31** (5.664
...
1. **El `UPDATE` de `sql/88` no toca ni una fila.** Exige `PRSTTSAA > 0` y los 7 candidatos tienen la
...
bloque `0.b` de `sql/88` no devolvió ninguna fila). O sea, **nadie apretó todavía el botón dos
...
antes de dar por buena la premisa (`sql/88`, bloque de diagnóstico 3).

---

## 5. Aviso al árbitro del EQUIPO A

**Rebajado el 2026-08-31 después de medir contra la base.** El aviso original decía que `sql/88`
...
Si en producción el bloque 0 de `sql/88` diera números distintos a los de local, el aviso vuelve a
...
| 3 | **`sql/88`** — correr el **bloque 0** en producción. El `UPDATE` es no-op en local; ejecutarlo solo si allá los números difieren | usuario | nada |
