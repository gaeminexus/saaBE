...
tipoCertificado, entidad, prestamo, calidad, fechaEmision [aaaa,m,d], usuarioEmision, datos (String
JSON), estado, usuarioAnulacion, fechaAnulacion, motivoAnulacion, fechaRegistro`.
**`pdf` no viaja** (`@JsonIgnore`).

**Forma exacta de tres campos** (verificada contra `model/crd/Certificado.java` el 2026-08-29, no
deducida — el contrato no lo decía y el frontend tuvo que preguntar):

| Campo | Mapeo | Qué llega |
|---|---|---|
| `tipoCertificado` | `@Basic Long` sobre `CRTFTPCR` | **número plano** (1..6, alterno del rubro 244), no un objeto |
| `entidad` | `@ManyToOne` sobre `ENTDCDGO` | el **objeto `Entidad` completo** |
| `prestamo` | `@ManyToOne` sobre `PRSTCDGO` | el **objeto `Prestamo` completo**; `null` salvo en el tipo 3 |

⚠️ Las entidades JPA se serializan directo a JSON en este proyecto (sin capa de DTO), así que
`entidad` y `prestamo` llegan enteros y arrastran sus propias relaciones. **Leer solo lo que se va a
mostrar**, sin asumir la profundidad del objeto. Para el número de crédito: `prestamo.idAsoprep` con
fallback a `prestamo.codigo` — la misma regla `NVL(PRSTIDAS, PRSTCDGO)` de
`PrestamoCertificado.numeroCredito`.

⚠️ **`Prestamo.idAsoprep` es el número de OPERACIÓN del préstamo.** No confundir con
`Aporte.idAsoprep`, que en esa otra entidad es el código de la carga Petro. Mismo nombre de campo,
