...
## 10. Implementación (2026-08-17)

Reporte implementado con el formato corporativo de los reportes `cnt`/`crd`
(mismo patrón que `RPRT_ESCT_APRT`: logo `P_IMAGEN`, recuadro de cabecera,
secciones grises, pie con paginación y leyenda institucional).

**Archivos creados:**

- `src/main/resources/rep/crd/RPRT_CMPB_PGCT.jrxml` — plantilla JasperReports (A4 horizontal).
  Se entrega **solo el `.jrxml`**; `ReporteServiceImpl` lo compila en runtime con Janino
  (no hay Jaspersoft Studio / `mvn` en el entorno para precompilar el `.jasper`).
- `src/main/java/com/saa/ws/rest/crd/ReportePagosCuotaRest.java` — endpoint dedicado.

**Decisiones de diseño (para no romper la compilación runtime en WildFly):**

- **Una sola query principal**, sin subdatasets ni componentes `list`/`table` (ningún
  reporte del repo los usa, así que ese camino no está probado bajo Janino). El maestro
  se une con `LEFT JOIN` a los pagos vigentes y los agregados (SUM por concepto, total
  pagado, nº de pagos) se traen como **subconsultas escalares**, de modo que cabecera,
...
```bash
# Endpoint dedicado (sugerido en §1):
GET /SaaBE/rest/crd/reportePagosCuota/{dtprCodigo}         -> application/pdf
GET /SaaBE/rest/crd/reportePagosCuota/{dtprCodigo}?usuario=jperez

# O el endpoint genérico de reportes:
POST /SaaBE/rest/rprt/generar
{ "modulo": "crd", "nombreReporte": "RPRT_CMPB_PGCT", "formato": "PDF",
  "parametros": { "P_DTPR_CODIGO": 12345 } }
```
