...
**Alcance: `cxp` · `cxc` · `pagos` · `tsr` · `rhh` · `sri`.** ⛔ **NO TOCAR: `crd`.**
...
### 🔴 Bloqueante
- **D2b — decidir dato vs. código en el secuencial fiscal**, con el contraste de numeración hecho
  primero. El árbitro escribe el `.sql` de diagnóstico; la decisión no le corresponde.

### 🟡 Decidible
- Correr la verificación de que `tsr/sql/07` y `08` están ejecutados.
- Frente O · T3 y frente 10b, los dos ya listados arriba con su bloqueo.

### ⛔ Antes de cualquier envío real al SRI
- Validar el XML del ATS contra el XSD y el validador oficial. Nunca se hizo.
