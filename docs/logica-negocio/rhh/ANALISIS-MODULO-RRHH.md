...
**Problemas de tipos:** dinero mapeado como `Double` en casi todas las entidades —lo que en su momento se consideró inaceptable
para nómina, aunque el análisis posterior mostró que a esta escala el error no se materializa; ver la regla 4 del maestro— salvo `ReglonNomina`, que usaba
`BigDecimal` y se uniformó a `Double`. Horas como `String` (`TRNOENTR`, `DTLLENTR`, `RSMNENTR`), lo que impide calcular
...
personal, aporte patronal, fondos de reserva y retención de IR del mes.
...
- Dinero: **se mantiene `Double`** en todo el módulo. Decisión del 2026-08-19 que revirtió la propuesta original de migrar a `BigDecimal`; la justificación completa está en la regla 4 del maestro. El redondeo se centraliza en `com.saa.ejb.rhh.util.RedondeoNomina`.
...
**Redondeo:** `RedondeoNomina.redondea(...)` en **cada renglón**, nunca solo al
...
con `RedondeoNomina` y ajustar la diferencia por redondeo (menor a 0,05) contra la línea de Sueldos
