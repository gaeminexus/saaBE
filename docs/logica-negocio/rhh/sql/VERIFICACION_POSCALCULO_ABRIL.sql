-- =====================================================
-- MODULO: RHH - ABRIL RECIEN CALCULADO, ANTES DE CONTRASTAR
...
-- Neto esperado 16.089,22. Cliente 15.914,22. Diferencia +175,00 CLAVADOS.
-- OJO: el 16.089,22 es PREDICCION, no un numero visto. Si no sale clavado,
-- el primer sospechoso es esa prediccion, no la replica. Y el discriminador
-- de verdad es el bloque 2 del contraste, nunca este total.
-- =====================================================
...
SELECT m.MPLDIDNT, n.NMNADITR AS DIAS, n.NMNATING AS INGRESOS,
  ...
  JOIN RHH.PRDN p ON p.PRDNCDGO = n.PRDNCDGO
  JOIN RHH.MPLD m ON m.MPLDCDGO = n.MPLDCDGO
 WHERE p.PRDNANOO = 2026 AND p.PRDNMSEE = 4 AND m.MPLDIDNT = '1004350904';
...
-- Esperado: rol 12 -> 687,05  ·  rol 13 -> 1015,14  ·  rol 14 -> 1300,00
-- =====================================================
...
SELECT 'CESANTE CON NOMINA', m.MPLDIDNT, m.MPLDAPLL, 1
  ...
  JOIN RHH.PRDN p ON p.PRDNCDGO = n.PRDNCDGO
  JOIN RHH.MPLD m ON m.MPLDCDGO = n.MPLDCDGO
 WHERE p.PRDNANOO = 2026 AND p.PRDNMSEE = 4 AND m.MPLDESTD = 4;
