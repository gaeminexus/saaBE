...
| **e2-53** | `sri/sql/e2-53-nota-de-venta-base-cero.sql` | ⚠️ **NO es lectura.** La nota de venta se declara con base **gravada** y debe ir en base 0%: pone `SUBCERO = SUBTOTAL` en las notas de venta sin IVA y completa su detalle con `CODIGOIVASRI = 0`. Es el hueco que el `e2-52` deja abierto (su criterio clasifica el código en NULL como gravado). `COMMIT` comentado, reverso comentado | 🔴 **PENDIENTE — urgente, 2026-09-21.** Arregla el ATS **sin necesidad de WAR**. ⚠️ Su `COMMIT` va comentado: **sin ejecutarlo no se guarda nada** (ver `e2-54`) |
| **e2-54** | `sri/sql/e2-54-quedo-aplicado-el-e2-52.sql` | Los BLOQUES 0 y 1 son lectura: contestan si el `UPDATE` del `e2-52` **quedó guardado** o se perdió por el `COMMIT` comentado — el DIMM del 2026-09-21 sigue mostrando las 8 compras de la eléctrica con la base 0% en 7,23 y el resto declarado como gravado, con IVA 0,00. El BLOQUE 2 reaplica el `UPDATE` sólo si hace falta | 🔴 **PENDIENTE — urgente, 2026-09-21.** Es el primero que hay que correr: dice si el problema es el dato o el XML viejo |
| **e2-55** | `sri/sql/e2-55-base-no-objeto-de-iva.sql` | ⚠️ **DDL + datos.** `PGS.FCTC.SUBNOOBJ`: la contribución a bomberos/basura de las planillas eléctricas deja de declararse como tarifa 0% y pasa a **no objeto de IVA** (criterio del auditor interno, correo 2026-09-19). Mueve lo ya cargado de `SUBCERO` a `SUBNOOBJ` y pone `CODIGOIVASRI = 6` en el detalle. **Va ANTES del WAR** — la entidad mapea la columna | 🔴 **PENDIENTE — 2026-09-21.** Orden estricto: `e2-55` → WAR → regenerar el ATS |
| **e2-56** | `sri/sql/e2-56-lineas-no-objeto-y-exento-del-xml.sql` | ¿Hay compras cuyo XML trae líneas con código 6 (no objeto) o 7 (exento)? Su base no va a ninguna columna de cabecera y el ATS la declara como **gravada**. Mismo defecto que el bomberos, por otro camino. **Solo lectura** | 🔴 **PENDIENTE — 2026-09-21** |
| **e2-57** | `sri/sql/e2-57-que-arreglos-nuestros-estan-realmente-en-la-base.sql` | ⭐ **El que hay que correr primero.** Para CADA arreglo de datos que entregó este equipo (`e2-45`, `e2-47`, `e2-52`/`e2-54`, `e2-53`, `e2-55`) dice si está **aplicado o perdido** — el `COMMIT` comentado hace que un `UPDATE` se pierda sin dejar rastro. Incluye el corte de las facturas mixtas. **Solo lectura** | 🔴 **PENDIENTE — urgente, 2026-09-21** |
| **e2-65** | `cxc/sql/e2-65-ventas-al-5-y-al-8-que-el-ats-no-declara.sql` | El ATS de ventas alimenta la gravada con `SUBTOTAL` y el IVA con `VIVA`, e **ignora `SUBTOTAL5`/`SUBTOTAL8`/`VIVA5`/`VIVA8`**: toda venta al 5% u 8% queda fuera del anexo. Mide si existen y en qué períodos. **Solo lectura** | 🔴 **PENDIENTE — 2026-09-21.** Si da cero filas, el defecto no afectó ninguna declaración |
