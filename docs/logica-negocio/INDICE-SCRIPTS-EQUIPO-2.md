...
> Última actualización: **2026-09-21** (e2-65; estados al día tras el despliegue del WAR). Al agregar un `e2-*` nuevo, agregar la fila acá **en el mismo
> commit**.
...
| **e2-53** | `sri/sql/e2-53-nota-de-venta-base-cero.sql` | ⚠️ **NO es lectura.** La nota de venta se declara con base **gravada** y debe ir en base 0%: pone `SUBCERO = SUBTOTAL` en las notas de venta sin IVA y completa su detalle con `CODIGOIVASRI = 0`. Es el hueco que el `e2-52` deja abierto (su criterio clasifica el código en NULL como gravado). `COMMIT` comentado, reverso comentado | ✅ **CORRIDO el 2026-09-21** según el usuario. ⚠️ Escribe: sin su bloque de control posterior no hay constancia de que el `UPDATE` quedara guardado (§46.1) |
| **e2-54** | `sri/sql/e2-54-quedo-aplicado-el-e2-52.sql` | Los BLOQUES 0 y 1 son lectura: contestan si el `UPDATE` del `e2-52` **quedó guardado** o se perdió por el `COMMIT` comentado — el DIMM del 2026-09-21 sigue mostrando las 8 compras de la eléctrica con la base 0% en 7,23 y el resto declarado como gravado, con IVA 0,00. El BLOQUE 2 reaplica el `UPDATE` sólo si hace falta | ✅ **CORRIDO el 2026-09-21.** Dio `YA CORREGIDA 18` y el bloque de control **vacío**: el `e2-52` quedó aplicado (§46.1) |
| **e2-55** | `sri/sql/e2-55-base-no-objeto-de-iva.sql` | ⚠️ **DDL + datos.** `PGS.FCTC.SUBNOOBJ`: la contribución a bomberos/basura de las planillas eléctricas deja de declararse como tarifa 0% y pasa a **no objeto de IVA** (criterio del auditor interno, correo 2026-09-19). Mueve lo ya cargado de `SUBCERO` a `SUBNOOBJ` y pone `CODIGOIVASRI = 6` en el detalle. **Va ANTES del WAR** — la entidad mapea la columna | ✅ **CORRIDO.** Confirmado el 21-09 por dos vías independientes: el `e2-63` consulta `SUBNOOBJ` sin error, y el WAR desplegado lee `FCTC` con la columna mapeada — si faltara, ORA-00904 (§49.5) |
| **e2-56** | `sri/sql/e2-56-lineas-no-objeto-y-exento-del-xml.sql` | ¿Hay compras cuyo XML trae líneas con código 6 (no objeto) o 7 (exento)? Su base no va a ninguna columna de cabecera y el ATS la declara como **gravada**. Mismo defecto que el bomberos, por otro camino. **Solo lectura** | ✅ **CORRIDO el 2026-09-21. TRAJO FILAS.** Agosto: 13 líneas código 6 (477,12) y 4 código 7 (0,39). De esas, 12 son bomberos ya cubiertas por el `e2-55`; las otras 5 (**400,39**: un reembolso de 400,00 y 0,39 de intereses por mora) las corrige el `e2-66`. Histórico: jul 448,21 · sep 420,00 |
| **e2-57** | `sri/sql/e2-57-que-arreglos-nuestros-estan-realmente-en-la-base.sql` | ⭐ **El que hay que correr primero.** Para CADA arreglo de datos que entregó este equipo (`e2-45`, `e2-47`, `e2-52`/`e2-54`, `e2-53`, `e2-55`) dice si está **aplicado o perdido** — el `COMMIT` comentado hace que un `UPDATE` se pierda sin dejar rastro. Incluye el corte de las facturas mixtas. **Solo lectura** | ✅ **CORRIDO el 2026-09-21** según el usuario. ⚠️ **Es de lectura y su salida no llegó al árbitro**, que es exactamente lo que este script existía para contestar (§49.4) |
| **e2-65** | `cxc/sql/e2-65-ventas-al-5-y-al-8-que-el-ats-no-declara.sql` | El ATS de ventas alimenta la gravada con `SUBTOTAL` y el IVA con `VIVA`, e **ignora `SUBTOTAL5`/`SUBTOTAL8`/`VIVA5`/`VIVA8`**: toda venta al 5% u 8% queda fuera del anexo. Mide si existen y en qué períodos. **Solo lectura** | ✅ **CORRIDO el 2026-09-21. CERO FILAS.** 52 facturas de venta en total y ninguna al 5% ni al 8% (tampoco con base 0%). El defecto del ATS de ventas es real pero **nunca afectó una declaración**: el arreglo de código va sin apuro |
| **e2-66** | `sri/sql/e2-66-no-objeto-y-exento-que-el-e2-55-no-cubrio.sql` | ⚠️ **NO es lectura.** Las 5 líneas no objeto/exento de agosto que el `e2-55` no cubre —identifica por descripción BOMBERO/BASURA—: un REEMBOLSO de 400,00 y 0,39 de intereses por mora, hoy declarados como base **gravada** de un documento sin IVA. Asigna `SUBNOOBJ`/`SUBEXENT` desde el detalle, idempotente y con guarda de gravada negativa. Septiembre incluido; julio comentado (ya declarado). **No necesita WAR** | 🔴 **PENDIENTE — urgente, 2026-09-21.** Su BLOQUE 3 tiene que dar CERO filas: es la única prueba de que el `COMMIT` entró |
| **e2-67** | `sri/sql/e2-67-verificacion-previa-a-generar-el-ats-de-agosto.sql` | ✅ **SOLO LECTURA.** Los cuatro números que hay que mirar ANTES de apretar Generar: si agosto tiene una compra con proveedor con **pasaporte** (el único defecto que puede hacer rechazar el anexo entero, §46.7), los totales de compra, la lista —que debe ser vacía— de facturas con base gravada e `IVA = 0`, las retenciones que nos hicieron, y las de intermediario que quedan fuera. Correr **después** del `e2-66` | 🔴 **PENDIENTE — 2026-09-21.** Su BLOQUE 0 decide si se genera o se para |
| **e2-68** | `cxp/sql/e2-68-anticipo-en-negativo-en-el-estado-de-cuenta.sql` | El Estado de Cuenta de un proveedor muestra «Saldo a favor (anticipos) **-1.200,00**» mientras la consulta de anticipos da 0,00, que es lo correcto. Los bloques 0-3 son **solo lectura**: localizan la fila negativa de `PGS.ANTP` que quedó sin marcar como MIGRADO, verifican que el cruce real exista en `PGS.APLP` y **cuentan la familia**. El BLOQUE 4 corrige y va **comentado** | 🔴 **PENDIENTE — 2026-09-22.** Correr los bloques 0-3 y pasar la salida al árbitro ANTES de descomentar el 4 |
| **e2-69** | `cxp/sql/e2-69-factura-de-prueba-fuera-de-solicitud-de-pago.sql` | La factura de prueba `001-501-000000203` (CUSTODIA & MAXIMA SEGURIDAD, 12.295,80) se sigue ofreciendo en Solicitud de Pago. Los bloques 0-2 son **solo lectura**: la identifican, dicen cuánto saldo real le queda y si tiene un pago vivo. El BLOQUE 3 la marca como PAGADA (`FCTCEPAG = 3`) y va **comentado**. Documenta además la opción B (anular), que NO hace por SQL | ✅ **CORRIDO el 2026-09-22.** La factura quedó con `FCTCEPAG = 3` y salió del combo de Solicitud de Pago. Los asientos de junio y julio, intactos. El saldo lo cerró después el `e2-70` |
| **e2-70** | `cxp/sql/e2-70-abono-de-regularizacion-factura-203.sql` | ⚠️ **NO es lectura.** El saldo de 11.975,04 de la factura 203 seguía figurando como «por pagar» en el Estado de Cuenta aunque la marcamos Pagada: el saldo lo calcula `total − aplicado`, no la marca. Registra el abono que falta, **sin asiento**, para que la factura cuadre sin tocar la contabilidad de junio. Su BLOQUE 1 **manda a parar** si `APLPASNT` no admite NULL | ✅ **CORRIDO y CONFIRMADO el 2026-09-22** por el usuario («ya corregí customax»). El abono de 11.975,04 sin asiento dejó el saldo en 0 sin tocar la contabilidad de junio |
| **e2-71** | `cxp/sql/e2-71-por-que-la-factura-de-aig-sigue-en-el-ats.sql` | La factura de AIG sigue saliendo en el ATS. **Solo lectura.** Lista TODAS las facturas de AIG con su período y su marca —hay al menos dos: la **470** de agosto, ya marcada antes del `e2-64`, y la **190** de julio, que no lo está— y contrasta el universo de marcadas contra las cinco que el `e2-64` dejó escritas. Separa las tres causas: XML viejo, marca perdida, u otra factura | ✅ **RESUELTO el 2026-09-23 sin correrlo.** El árbitro verificó sobre el XML real: AIG **no está** en el ATS (RUC `1790475247001`, cero apariciones), ni R| 🔴 **PENDIENTE — 2026-09-23** |M ni los dos secuenciales marcados de la Eléctrica. Las cuatro marcas funcionan. La factura a sacar era otra: ver `e2-72` |
| **e2-72** | `cxp/sql/e2-72-sacar-del-ats-la-factura-de-15283.sql` | ⚠️ **NO es lectura.** Saca del ATS la factura `004-100-003891553` (proveedor `1790007502001`, **15.283,98**, 26/08) marcándola como intermediario, sin tocar asiento ni contabilidad. Incluye **comentada aparte** la del mismo proveedor y día por 14,15, para que marcarla sea una decisión y no un arrastre | 🔴 **PENDIENTE — 2026-09-23.** Bloque 0 corrido: el proveedor es **EQUISUIZA** y son **CUATRO** facturas, no una. Decidido: se marcan sólo las **dos de agosto** (`468` y `469`); las dos de julio (30.722,23) **no se tocan** porque ese período ya se declaró. Motivo confirmado: intermediario. Falta el BLOQUE 2, su `COMMIT` y **regenerar** |
| **e2-73** | `cxp/sql/e2-73-desglose-de-la-343.sql` | ¿Qué le pasó al desglose de la 343 entre base 0% y base gravada? **Solo lectura.** Contrasta la cabecera de hoy contra el valor medido el 21-09 (`SUBCERO 1.441,17` / gravada `80,00`) y muestra el detalle línea por línea. La hipótesis es el `e2-66`, que reescribe `SUBNOOBJ`/`SUBEXENT` de toda factura de agosto con líneas código 6 o 7 | 🔴 **PENDIENTE — 2026-09-25** |
| **e2-64** | `cxp/sql/e2-64-sacar-del-ats-la-factura-343.sql` | ⚠️ **NO es lectura.** Marca como intermediario la factura 343 (`001-001-000002868`, R&M WORLDTRAVEL) para que **no salga del ATS ni del cuadre 104**, SIN tocar su asiento. Deja `FCTCPRIN` en nulo a propósito: si alguna vez se recontabiliza, correr antes el `lap1-09` | ✅ **CORRIDO y COMMITEADO el 2026-09-21.** `FCTCESIN = 1`, todo lo demás intacto |
| **e2-63** | `cxp/sql/e2-63-por-que-la-factura-de-intermediario-sigue-en-el-ats.sql` | Separa las tres causas por las que una factura marcada seguía en el ATS: XML viejo, marca nunca guardada, o WAR anterior al 15-09. **Solo lectura** | ✅ **CORRIDO el 2026-09-21.** Respuesta: `FCTCESIN = 0`, la marca nunca se guardó. Y confirmó que el `e2-60` sí está aplicado |
| **e2-62** | `cxp/sql/e2-62-recalculo-historico-de-las-bases-por-tarifa.sql` | ⚠️ **NO es lectura, y cambia períodos ya declarados.** Recalcula los cinco buckets desde el detalle, sólo en la factura cuyo detalle está completo y cierra contra el `SUBTOTAL`. El bloque 0 muestra el impacto mes por mes antes de tocar nada | ✅ **CORRIDO el 2026-09-21** según el usuario. ⚠️ Escribe y **cambia períodos ya declarados**: falta su bloque de control posterior (§49.4) |
| **e2-61** | `cxp/sql/e2-61-columnas-por-tarifa-en-los-documentos-de-compra.sql` | ⚠️ **DDL.** Las trece columnas por tarifa que faltaban en `FCTC`, `NTCC`, `NTDC` y `LQCC`. **Va antes del WAR** | ✅ **CORRIDO el 2026-09-21.** Las cuatro tablas con su juego completo |
| **e2-60** | `cxp/sql/e2-60-facturas-mixtas-de-agosto.sql` | ⚠️ **NO es lectura.** Las mixtas de agosto: corrige sólo la factura cuyo detalle cierra contra el subtotal. Deja afuera las de redondeo y la 423, cuyo detalle no tiene códigos | ✅ **CORRIDO y COMMITEADO el 2026-09-21.** 343, 493 y 494: 1.454,47 que salieron de la columna equivocada |
| **e2-59** | `cxp/sql/e2-59-inventario-de-tarifas-en-las-compras.sql` | Qué tarifas aparecen de verdad en el detalle de las compras, y si `SUBTOTAL5`/`SUBTOTAL8` están vacías. Define el tamaño del frente de clasificación. **Solo lectura** | ✅ **CORRIDO el 2026-09-21** según el usuario. ⚠️ **Es de lectura y su salida no llegó al árbitro** |
| **e2-58** | `cxp/sql/e2-58-total-de-las-retenciones-ya-cargadas.sql` | ⚠️ **NO es lectura.** Las retenciones recibidas ya cargadas se ven con Total 0,00 en Consulta de Documentos: el TXT del SRI las trae en cero y el total retenido vive en `RCV2.TOTAL`/`RTCM.TOTAL`. Les pone su total. El código nuevo (`35cf5ecc`) ya lo hace para las que entren de ahora en adelante | ✅ **CORRIDO el 2026-09-21** según el usuario. ⚠️ Escribe: falta su bloque de control posterior (§49.4) |
