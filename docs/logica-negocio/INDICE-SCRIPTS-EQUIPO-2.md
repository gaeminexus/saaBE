...
| **e2-69** | `cxp/sql/e2-69-factura-de-prueba-fuera-de-solicitud-de-pago.sql` | La factura de prueba `001-501-000000203` (CUSTODIA & MAXIMA SEGURIDAD, 12.295,80) se sigue ofreciendo en Solicitud de Pago. Los bloques 0-2 son **solo lectura**: la identifican, dicen cuánto saldo real le queda y si tiene un pago vivo. El BLOQUE 3 la marca como PAGADA (`FCTCEPAG = 3`) y va **comentado**. Documenta además la opción B (anular), que NO hace por SQL | 🔴 **PENDIENTE — 2026-09-22.** Bloques 0-2 ya corridos: saldo real **11.975,04**, asiento 7752, una retención de 320,76 y un pago **477 en POR_APROBAR**. Decisión del usuario: anular el pago 477 en Tesorería y correr el BLOQUE 3. Los asientos **no se tocan** |
| **e2-70** | `cxp/sql/e2-70-abono-de-regularizacion-factura-203.sql` | ⚠️ **NO es lectura.** El saldo de 11.975,04 de la factura 203 seguía figurando como «por pagar» en el Estado de Cuenta aunque la marcamos Pagada: el saldo lo calcula `total − aplicado`, no la marca. Registra el abono que falta, **sin asiento**, para que la factura cuadre sin tocar la contabilidad de junio. Su BLOQUE 1 **manda a parar** si `APLPASNT` no admite NULL | 🟡 **BLOQUES 0 y 1 CORRIDOS el 2026-09-22 — VERDE.** `APLPASNT` es `NULLABLE = Y`, las seis obligatorias (`APLPCDGO`/`APLPPJRQ`/`APLPTDPG`/`APLPMAPL`/`APLPFAPL`/`APLPESTD`) las llena el INSERT, y la secuencia va en 304 con la tabla en 303. **Falta el BLOQUE 2 y su `COMMIT`** |
