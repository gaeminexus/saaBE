...
> Última actualización: **2026-09-07**. Al agregar un `e2-*` nuevo, agregar la fila acá **en el mismo
> commit**.
...
| **e2-10** | **`cxp/sql/e2-10-retenciones-cargadas-con-cuenta-de-proveedor.sql`** | Retenciones ya cargadas con la cuenta de proveedor de un cliente (bloque A2) y titulares sin cuenta de cliente (B1). **Solo lectura** | 🟡 **PENDIENTE**: sólo hace falta el **A2** |
...
|---|---|---|
| **`e2-10`, sólo el bloque A2** | Si hay retenciones que ya generaron asiento con la cuenta equivocada. **Lo más probable es que dé 0 filas** y el tema se cierre | ninguno, es lectura |
