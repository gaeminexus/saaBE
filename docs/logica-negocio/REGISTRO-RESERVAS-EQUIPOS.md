...
> ✅ **Reconfirmado por el usuario el 2026-09-08**, palabras suyas: *"la pdtr no usa secuencia ni la
> prbr"*. Se puso en duda ese día porque `tsr/sql/e2-13` está escrito con `SCP.SQ_PDTRCDGO.NEXTVAL`
> y sus tres filas SÍ están en la base — pero eso no prueba que la secuencia exista: prueba que el
| 2026-09-08 | rhh/cxp/pagos/cnt/tsr (arbitro `omen-saa-2`) | **`PDTR` 1516** — detalle **36** (`VACACIONES GOZADAS`) del rubro **221** | Script `rhh/sql/e2-29`. Le da rol de motor al concepto de alterno 12 (`Vacaciones pagadas`) para que el rol **debite la provisión** en vez de mandar todo a gasto. ⚠️ **El script pedía el 1511 y en la base quedó el 1516.** Medido: **no hay trigger** —los 1500-1503 del `e2-03` quedaron donde se escribieron—; el corrimiento de cinco lugares lo produjo `tsr/sql/e2-13`, que tomó **1504-1506**, más **1507-1508** del `e2-18`. **Numeración real medida el 2026-09-08:** 1500-1503 `e2-03` · 1504-1506 `e2-13` (rubro 71) · 1507-1508 `e2-18` · 1509-1513 `e2-26` rubro 311 · 1514-1515 `e2-26` detalles 34/35 · 1516 `e2-29`. Todo dentro del bloque 1500-1599, **sin invadir a nadie**. 🔴 **Los reversos de `e2-26` y `e2-29` apuntaban a los números viejos y habrían borrado los comandos de búsqueda del rubro 71** (paréntesis e `IS NULL`), rompiendo `selectByCriteria` en todo el sistema — corregidos. 🔴 **Y el `e2-13` estaba escrito con `SCP.SQ_PDTRCDGO.NEXTVAL`**, una secuencia que **no existe** (§1bis, reconfirmado por el usuario: *"la pdtr no usa secuencia ni la prbr"*): o sea que ese script **no se corrió como estaba commiteado**, y su cabecera afirmaba además que "no consume ningún rango reservado" cuando sí consumió tres códigos de este equipo — corregido. **Lección: un script que toma el PK de otro lado no aparece en este registro y desplaza a todos los que vienen después.** |
> regla sigue siendo: PK explícita, del rango del equipo.**
...
| 2026-09-07 | cxp/cxc/pagos/tsr/rhh/sri/cnt (árbitro `lap-saa-1-arb`, **laptop**) | **`PRBR` 330** + **`PDTR` 1600-1603**, y las tablas **`RHH.PLIS`** y **`RHH.DLIS`** | Rubro `RHH_TIPO_PLANILLA_IESS` — `ROL NORMAL(1)`, `PRESTAMOS QUIROGRAFARIOS(2)`, `PRESTAMOS HIPOTECARIOS(3)`, `FONDOS DE RESERVA(4)`. **Primeros códigos usados del bloque 330-349 / 1600-1699**, reservado por este equipo el 2026-09-01 sin gastar ninguno hasta hoy. Se aplica la §6: `PRBRALTR = PRBRCDGO = 330`. El IESS emite **una planilla por rubro y por período**, no una sola (precisión del usuario, 2026-09-07), y cada una tiene su comprobante y su pago: de ahí el rubro. `PLIS` es la planilla **emitida por el portal** que se captura para conciliar — **no confundir con el `@Path("plie")`**, que es la planilla de *control* y es un POJO calculado, sin tabla. Las dos verificadas libres contra `src/main/java/com/saa/model/`, contra `docs/` y contra este registro — **falta confirmarlas contra `ALL_TABLES`**, y ese control es el **1** del propio DDL, que detiene el script si devuelve filas. El `MAX` de `PRBR`/`PDTR` se revalida con el usuario justo antes de ejecutar (regla 2), en el **CONTROL 2** del script. DDL en `rhh/sql/lap1-08-planilla-iess.sql`, diseño en `rhh/API-PLANILLA-IESS.md`. ⚠️ **`rhh` es alcance compartido con `omen-saa-2`**: estas tablas son nuevas y no tocan lo suyo, pero el **pago** (Fase 2) puede necesitar `PagoProgramadoServiceImpl`, que sí es territorio común — no se toca sin coordinar entre árbitros |
| 2026-09-08 | rhh/cxp/pagos/cnt/tsr (arbitro `omen-saa-2`) | **`PRBR` 311** + **`PDTR` 1504-1508**, y la tabla **`RHH.VNPG`** | Rubro nuevo `RHH_ESTADO_VALOR_NO_PAGADO` -- `REGISTRADO(1)`, `RETENIDO(2)`, `PAGADO(3)`, `ANULADO(4)`, `FINIQUITADO(5)` -- y la tabla de **valores no pagados en nomina**: un valor que se retiene del pago de un mes y se devuelve en el pago del siguiente, sin tocar la contabilidad del rol ni las provisiones (solo mueve remuneraciones por pagar, y solo en el pago). Segundo rubro del bloque 310-329 / 1500-1599 de este equipo (el primero fue el 310 del 2026-09-01). Se aplica la §6: `PRBRALTR = PRBRCDGO = 311`. El `MAX` de `PRBR`/`PDTR` se revalida con el usuario **justo antes** de ejecutar (regla 2), en el BLOQUE 0 del DDL. **Consume ademas los detalles 34 y 35 del rubro 221** (`RHH ROL DEL CONCEPTO EN EL MOTOR`), por `NEXTVAL` como los 32/33 de los decimos, no del bloque. `VNPG` verificada libre contra `src/main/java/com/saa/model/` (cero coincidencias entre las 56 tablas `RHH` mapeadas) y contra este registro -- **falta confirmarla contra `ALL_TABLES`**, y ese control es el **0.1 del propio DDL**, que detiene el script si devuelve filas. Diseño en `rhh/PLAN-VALORES-NO-PAGADOS.md`, DDL en `rhh/sql/e2-26`. ⚠️ `rhh` es alcance compartido con `lap-saa-1`: la tabla es nueva y no toca lo suyo, pero el cambio engancha en `GeneracionOrdenPagoServiceImpl` y `ProcesoNominaServiceImpl`, que son comunes -- se les avisa antes del merge |
| 2026-09-08 | rhh/cxp/pagos/cnt/tsr (arbitro `omen-saa-2`) | **`PDTR` 1516** — detalle **36** (`VACACIONES GOZADAS`) del rubro **221** | Script `rhh/sql/e2-29`. Le da rol de motor al concepto de alterno 12 (`Vacaciones pagadas`) para que el rol **debite la provisión** en vez de mandar todo a gasto. ⚠️ **El script pedía el 1511 y la base asignó el 1516.** Medido: **no hay trigger** —los 1500-1503 del `e2-03` quedaron exactamente donde se escribieron—; el corrimiento de cinco lugares lo produjo el `tsr/sql/e2-13`, que tomó **1504-1506** con `SCP.SQ_PDTRCDGO.NEXTVAL`, más **1507-1508** del `e2-18`. **Numeración real medida el 2026-09-08:** 1500-1503 `e2-03` · 1504-1506 `e2-13` (rubro 71) · 1507-1508 `e2-18` · 1509-1513 `e2-26` rubro 311 · 1514-1515 `e2-26` detalles 34/35 · 1516 `e2-29`. Todo dentro del bloque 1500-1599, **sin invadir a nadie**. 🔴 **Los reversos de `e2-26` y `e2-29` apuntaban a los números equivocados y habrían borrado los comandos de búsqueda del rubro 71** (paréntesis e `IS NULL`), rompiendo `selectByCriteria` en todo el sistema — corregidos el 2026-09-08. 🔴 **Y contradice el §1bis:** `e2-13` usó `SCP.SQ_PDTRCDGO.NEXTVAL` y sus filas existen, así que la secuencia **sí existe** o fue creada después de aquella verificación como `SYS` — pendiente de confirmar con `ALL_SEQUENCES` antes de reescribir el §1bis |
...
| 2026-09-21 | `omen-saa-1` (CRD) | **Tabla `CRD.CBBP`** — beneficiarios del partícipe, con su cuenta bancaria y su porcentaje | Requerimiento del usuario del 2026-09-21: cuando un partícipe fallece, la aseguradora entrega al fondo un valor de sepelio que el fondo debe repartir **entre los familiares**, según un porcentaje, a la cuenta bancaria de cada uno y con su certificado bancario digitalizado. Hoy no hay dónde guardar eso: `CRD.CNBP` es la cuenta bancaria **del propio partícipe** (no tiene nombre ni cédula, porque el titular es él) y `validarCuentaBancariaParticipe` exige que la cuenta le pertenezca y esté activa — la de un fallecido no sirve. Una fila = un beneficiario con su cuenta; el certificado NO es columna, va en `CRD.ADJN` con el tipo `CRD.TPDJ` «CERTIFICADO BANCARIO», igual que `CuentaBancariaParticipeServiceImpl.crearConCertificado`. Verificado libre contra los `@Table` de `src/main/java/com/saa/model/` (cero ocurrencias; también quedan libres `BFPR`, `BNPR`, `BFCT`, `CBBN`) — **falta el control contra `ALL_TABLES`**, que va como bloque 0 del propio DDL y detiene el script si devuelve filas. **No consume ningún `PRBR`/`PDTR`.** Diseño en `crd/DISENO-BENEFICIARIOS-Y-VALORES-DE-SEGURO.md`. ✅ **AUTORIZADA POR EL USUARIO el 2026-09-22**, no sólo reservada — cerrado el ítem S1 del diseño. DDL en `crd/sql/233_DDL_BENEFICIARIOS_PARTICIPE.sql` (control `ALL_TABLES` como bloque 0.1, índice único **`(ENTDCDGO, CBBPIDNT)`** y no sobre la cédula sola), contrato en `crd/API-BENEFICIARIOS-PARTICIPE.md`. ⚠️ **El DDL cubre sólo la fase 2a** (la tabla y su CRUD): el **pago** a los beneficiarios sigue bloqueado por el producto de pago de CXP contra `2.3.90.90.11`, que es de `omen-saa-2` |
| 2026-09-21 | `omen-saa-1` (CRD) | **Tabla `CRD.RVSG`** — recepción de valores de seguro que el fondo recibe para entregar a beneficiarios | Cuando un partícipe fallece, la aseguradora entrega **al fondo** un valor de sepelio que el fondo reparte después entre los familiares. Hoy ese dinero llega al banco y **no queda ni asentado ni asignado al partícipe**. Ciclo calcado de `CRD.CBCR`: se registra, contabilidad aprueba, y recién ahí se genera el asiento **D banco / H 2.3.90.90.11** y el valor entra al saldo. ⚠️ **No lleva columna de empresa a propósito**: se deriva de la cuenta bancaria (`CNBC`→`PLNN`→`PJRQ`), igual que `derivarEmpresaCobro`; guardarla sería una segunda fuente de verdad que puede discrepar en silencio. Verificada libre contra los `@Table` de `src/main/java/com/saa/model/` (cero ocurrencias; también quedan libres `RCSG`, `VSGR`, `RSGR`) — el control contra `ALL_TABLES` es el bloque 0.1 del propio DDL y detiene el script si devuelve filas. **No consume ningún `PRBR`/`PDTR`**: los cuatro estados (registrado/aprobado/rechazado/anulado) son constantes planas, estado técnico de proceso y no parametría de oficina. DDL en `crd/sql/230_DDL_RECEPCION_VALORES_SEGURO.sql`, configuración en `crd/sql/231`. Contrato en `crd/API-RECEPCION-VALORES-SEGURO.md`. ✅ **AUTORIZADA POR EL USUARIO el 2026-09-21** («está bien la tabla RVSG»), no sólo reservada |
